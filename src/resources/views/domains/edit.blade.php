...
@section('content')
    <div class="max-w-2xl mx-auto">
        <h1 class="text-3xl font-bold mb-6">Редактировать домен</h1>
        ...
        <div class="bg-white p-8 rounded-lg shadow-md">
            ...
            <form action="{{ route('domains.update', $domain) }}" method="POST">
                ...
                <div class="mb-4">
                    <label for="domain" class="block text-gray-700 font-bold mb-2">Название домена</label>
                    <input type="text"
                           name="domain"
                           id="domain"
                ...
                @php $setting = $domain->checkSetting; @endphp
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="check_interval_minutes" id="check_interval_minutes"
                           value="{{ old('check_interval_minutes', $setting?->check_interval_minutes ?? 5) }}" min="1" max="1440"
                           class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="request_timeout_seconds" id="request_timeout_seconds"
                           value="{{ old('request_timeout_seconds', $setting?->request_timeout_seconds ?? 10) }}" min="1" max="30"
                           class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="check_method" id="check_method"
                            class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500">
                        <option value="GET" {{ old('check_method', $setting?->check_method ?? 'GET') === 'GET' ? 'selected' : '' }}>GET</option>
                        <option value="HEAD" {{ old('check_method', $setting?->check_method ?? 'GET') === 'HEAD' ? 'selected' : '' }}>HEAD</option>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    <label class="flex items-center">
                        ...
                        <input type="checkbox" name="auto_checks_enabled" value="1"
                               {{ old('auto_checks_enabled', $setting?->auto_checks_enabled ?? true) ? 'checked' : '' }}
